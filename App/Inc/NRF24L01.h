...
/* 移植说明:
 * 原代码: 正点原子 NRF24L01 驱动 (寄存器操作 SPI + sys.h)
 * 本版本: 移植到 HAL 库 + CubeMX 工程, SPI2 硬件 SPI (PB13 SCK / PB14 MISO / PB15 MOSI)
 * 底层字节交换使用 HAL_SPI_TransmitReceive, GPIO 使用 HAL_GPIO_*
 */

#include "main.h"
#include "spi.h"		/* 提供 extern SPI_HandleTypeDef hspi2 */

//////////////////////////////////////////////////////////////////////////////////////////////////////////
//NRF24L01寄存器操作命令
//注: 不用 READ_REG/WRITE_REG 命名, 避免与 CMSIS 头文件 (stm32f1xx.h) 中的
//    READ_REG(REG)/WRITE_REG(REG,VAL) 宏重名产生告警
 ...
 * 本项目 PA3/PA4 已被按键占用, PA5 为 LED, 故默认改为:
 *   CE  = PB0   (推挽输出)
 *   CSN = PB1   (推挽输出, 低有效)
...
//发送等待 IRQ 超时时间(ms), 防止模块损坏/未连接时死等
#define NRF24L01_TX_TIMEOUT  100

void NRF24L01_Init(void);                              //初始化(含 GPIO 与 CSN/CE 默认电平)
...
uint8_t NRF24L01_Check(void);                          //检查24L01是否存在, 0:成功, 1:失败
